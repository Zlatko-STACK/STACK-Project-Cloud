...
def render_calendar(projects_df):
        ...
        for _, _p in projects_df.iterrows():
            for _f in ("Start date", "Target completion"):
                ...
                if _v is not None and _v.year == y and _v.month == m:
                    ...
                    _lbl = str(_p.get("Project name", "") or "").strip()
                    _lbl = (_lbl + " \u2014 " + _f) if _lbl else _f
                    marked_labels.setdefault(int(_v.day), []).append(_lbl)
    except Exception:
        pass
    ...
    for week in _cal.Calendar(firstweekday=0).monthdayscalendar(y, m):
        ...
        for day in week:
            ...
            _tip = "; ".join(marked_labels.get(day, [])) if day in marked else ""
            _tip_attr = (" title=\"" + _tip.replace('"', "&quot;") + "\"") if _tip else ""
            _cur = "cursor:help;" if _tip else ""
            cells += f"<td style='padding:4px 0;text-align:center;{_cur}'{_tip_attr}>{num}{dot}</td>"
        rows += f"<tr>{cells}</tr>"
...
def parse_milestones_struct(value):
    ...
    for line in parse_milestones(value):
        ...
        out.append({"id": create_id(), "text": text, "done": done, "date": ms_date})
    return out
...
def serialize_milestones_struct(items):
    def _line(it):
        ...
        d = str(d).strip()
        if d: t = f"{t} @{d}"
        return f"{t} ✓" if it.get("done") else t
    return "\n".join(_line(it) for it in items if (it.get("text") or "").strip())
...
def nz_public_holidays(year):
    ...
    kings_birthday = nth_weekday(year, 6, 0, 1)
    labour_day = nth_weekday(year, 10, 0, 4)
    hols = [
...
def get_week_starts(offset=0, count=17, holiday_dates=None):
    ...
    for i in range(count):
        ...
        weeks.append({
            "start": ws, "start_str": ws.strftime("%Y-%m-%d"),
            "label": f"W{ws.isocalendar()[1]}", "date_label": ws.strftime("%d/%m/%Y"),
            "month": wed.strftime("%B %Y"), "workdays": week_workdays(ws, holiday_dates),
            "is_current": (ws == monday),
        })
    return weeks

def member_week_leave_days(week_start, member_leave_dates, holiday_dates):
    count = 0
    for i in range(5):
        d = (week_start + pd.Timedelta(days=i)).date()
        if d in holiday_dates: continue
        if d in member_leave_dates: count += 1
    return count

def member_week_capacity(week, base_capacity, member_leave_dates, holiday_dates):
    leave_days = member_week_leave_days(week["start"], member_leave_dates, holiday_dates)
    eff = max(0, week["workdays"] - leave_days)
    return base_capacity * (eff / 5.0), eff, leave_days

def avail_color(hours, capacity=40.0):
    if hours < 0:  return ("#c0392b", "#ffffff")
    if hours == 0: return ("#f4f6f7", "#888888")
    pct = (hours / capacity) if capacity > 0 else 0
    if pct > 0.75: return ("#27ae60", "#ffffff")
    if pct > 0.5:  return ("#52be80", "#ffffff")
...
def _current_week_availability(member_names):
    # Returns a list of dicts with each member's capacity / allocated / free hours
    # for the current week, reusing the same logic as the Resourcing page.
    import pandas as _pd
    today = _pd.Timestamp.now().normalize()
    cur_monday = today - _pd.Timedelta(days=today.weekday())
    week_str = cur_monday.strftime("%Y-%m-%d")
    hol = holiday_dates_set(st.session_state.get("holidays"))
    wk = {"start": cur_monday, "workdays": week_workdays(cur_monday, hol)}
    cap = float(st.session_state.get("res_capacity", 40.0))
    allocs = st.session_state.get("resource_allocs")
    leave = st.session_state.get("leave")
    rows = []
    for m in member_names:
        wcap, _u1, _u2 = member_week_capacity(wk, cap, leave_dates_for_member(m, leave), hol)
        planned = member_week_planned(m, week_str, allocs)
        free = round(wcap - planned, 1)
        rows.append({"member": m, "capacity": wcap, "planned": planned, "free": free})
    rows.sort(key=lambda r: r["free"], reverse=True)
    return rows, cur_monday


def render_capacity_popover(member_names, key_suffix):
    # A small "pop up" (st.popover) that shows who has capacity this week,
    # based on the resourcing tracker. Safe to use inside st.form.
    try:
        rows, cur_monday = _current_week_availability(member_names)
    except Exception:
        st.caption("Capacity data unavailable.")
        return
    with st.popover("👥 Who has capacity?", use_container_width=True):
        st.markdown(f"**Capacity — week of {cur_monday.strftime('%d %b %Y')}**")
        st.caption("Free hours = weekly capacity − allocated (projected) hours from the resourcing tracker.")
        if not rows:
            st.info("No team members found.")
            return
        html = "<div style='font-size:12px'>"
        for r in rows:
            bg, fg = avail_color(r["free"], r["capacity"] if r["capacity"] else 40.0)
            label = "free" if r["free"] > 0 else ("full" if r["free"] == 0 else "over")
            html += (
                "<div style='display:flex;justify-content:space-between;align-items:center;"
                "padding:4px 8px;margin-bottom:3px;border-radius:4px;background:#f6f6f6'>"
                f"<span style='font-weight:600'>{r['member']}</span>"
                f"<span style='display:inline-block;padding:1px 8px;border-radius:10px;"
                f"background:{bg};color:{fg};font-weight:600'>{fmt_hours(r['free'])}h {label}</span>"
                f"<span style='color:#888'>{fmt_hours(r['planned'])}/{fmt_hours(r['capacity'])}h</span>"
                "</div>"
            )
        html += "</div>"
...
def build_availability_table_html(member_names, weeks, allocs_df, leave_df, holiday_dates, capacity=40.0):
    ...
    dark = "background:#2c2c2c"; cur = "background:#e8a87c;color:#1a1a1a"
...
def client_billing_summary(company_id, invoices_df, lines_df):
    ...
    return {"total_invoiced": round(total_invoiced, 2), "paid": round(paid, 2), "outstanding": round(total_invoiced - paid, 2)}
...
def build_gantt_chart(df):
    if df.empty:
        st.info("No projects to display."); return
    STATUS_COLOURS = {"On track": "#2ecc71", "At risk": "#f39c12", "Delayed": "#e74c3c", "Complete": "#95a5a6"}
    today = pd.Timestamp.now().normalize()
    rows = []
    for _, proj in df.iterrows():
        phases = parse_phase_schedule(proj.get("Phase schedule", ""))
        if not phases: continue
        status = proj.get("Status", "On track"); colour = STATUS_COLOURS.get(status, "#3498db")
        for p in phases:
            rows.append({"Project": proj["Project name"], "Phase": p["Stage"], "Start": p["Start date"],
                         "End": p["Target completion"], "Status": status, "Colour": colour})
    if not rows:
        st.info("No phase schedules found. Add phase schedules to your projects to see the Gantt chart."); return
    gantt = pd.DataFrame(rows)
    gantt["Start"] = pd.to_datetime(gantt["Start"]); gantt["End"] = pd.to_datetime(gantt["End"])
    ...
    y_labels = []; prev_proj = None; display_rows = []
    for _, row in gantt.iterrows():
        if row["Project"] != prev_proj:
            y_labels.append(f"▸ {row['Project']}"); prev_proj = row["Project"]
        phase_label = f"{row['Project']} | {row['Phase']}"
        y_labels.append(phase_label)
        display_rows.append({**row.to_dict(), "y_label": phase_label})
    disp_df = pd.DataFrame(display_rows)
    # --- milestone markers (subtle diamonds on each project header row) ---
    ms_marker_rows = []
    for _, mrow in df.iterrows():
        m_items = parse_milestones_struct(mrow.get("Milestones", ""))
        m_ylabel = f"▸ {mrow['Project name']}"
        for m in m_items:
            md = parse_date(m.get("date"))
            if md is None:
                continue
            ms_marker_rows.append({
                "y_label": m_ylabel,
                "Date": pd.to_datetime(md),
                "Milestone": m.get("text") or "(untitled)",
                "Status": "Done" if m.get("done") else "Pending",
                "DateStr": md.strftime("%d %b %Y"),
            })
    ms_df = pd.DataFrame(ms_marker_rows)
    ms_layer = None
    if not ms_df.empty:
        ms_layer = alt.Chart(ms_df).mark_point(
            shape="diamond", size=85, filled=True, opacity=0.9,
            stroke="#ffffff", strokeWidth=1,
        ).encode(
            x=alt.X("Date:T"),
            y=alt.Y("y_label:N", sort=y_labels),
            color=alt.Color("Status:N", scale=alt.Scale(domain=["Pending", "Done"], range=["#6c5ce7", "#2ecc71"]), legend=None),
            tooltip=[alt.Tooltip("Milestone:N", title="Milestone"), alt.Tooltip("DateStr:N", title="Date"), alt.Tooltip("Status:N", title="Status")],
        )
    bars = alt.Chart(disp_df).mark_bar(height=16, cornerRadius=4).encode(
        ...
        x2=alt.X2("End:T"),
        y=alt.Y("y_label:N", sort=y_labels, title="", axis=alt.Axis(labelFontSize=12, labelLimit=220, ticks=False, domain=False)),
        color=alt.Color("Status:N", scale=alt.Scale(domain=list(STATUS_COLOURS.keys()), range=list(STATUS_COLOURS.values())), legend=alt.Legend(title="Status", orient="top", titleFontSize=11, labelFontSize=11)),
        tooltip=[alt.Tooltip("Project:N"), alt.Tooltip("Phase:N"), alt.Tooltip("Start:T", format="%d %b %Y"), alt.Tooltip("End:T", format="%d %b %Y"), alt.Tooltip("Status:N")])
    ...
    if header_rows:
        ...
        header_text = alt.Chart(hdr_df).mark_text(align="left", dx=-220, fontSize=12, fontWeight="bold", color="#2c3e50").encode(y=alt.Y("y_label:N", sort=y_labels), text=alt.Text("y_label:N"))
        _layers = [header_bands, bars, bar_labels, today_line, today_label, header_text]
        if ms_layer is not None: _layers.append(ms_layer)
        chart = alt.layer(*_layers)
    else:
        _layers = [bars, bar_labels, today_line, today_label]
        if ms_layer is not None: _layers.append(ms_layer)
        chart = alt.layer(*_layers)
    n_rows = len(y_labels); chart_height = max(500, n_rows * 32 + 120)
    chart = chart.properties(width="container", height=chart_height, background="transparent").configure_view(strokeWidth=0, fill="transparent").configure_axis(labelColor="#2c2c2c", titleColor="#2c2c2c", domainColor="#c9c6c1", gridColor="#e3e1dd", tickColor="#c9c6c1").configure_legend(labelColor="#2c2c2c", titleColor="#1a1a1a").configure_title(color="#1a1a1a")
...
[data-testid="stSidebar"] .stButton > button { background-color: #ffffff !important; color: #1a1a1a !important; border: 1px solid #1a1a1a !important; border-radius: 4px !important; font-weight: 600 !important; letter-spacing: 0.01em !important; transition: all 0.15s ease !important; }
[data-testid="stSidebar"] .stButton > button:hover { background-color: #F2C94C !important; color: #1a1a1a !important; border: 1px solid #1a1a1a !important; }
[data-testid="stSidebar"] .stButton > button div, [data-testid="stSidebar"] .stButton > button p, [data-testid="stSidebar"] .stButton > button span { color: #1a1a1a !important; }
[data-testid="stSidebar"] .stButton > button:hover div, [data-testid="stSidebar"] .stButton > button:hover p, [data-testid="stSidebar"] .stButton > button:hover span { color: #1a1a1a !important; }
...
/* ICON-FONT-FALLBACK-V1: fallback when Material Symbols icon webfont fails to load (prevents raw 'arrow_drop_down'/'expand_more' text in expanders & popovers) */
[data-testid="stIconMaterial"]{ font-size:0 !important; line-height:1 !important; display:inline-flex !important; align-items:center; justify-content:center; vertical-align:middle; }
[data-testid="stIconMaterial"]::after{ content:"▾" !important; font-size:1.1rem !important; color:inherit; font-family:inherit !important; }
</style>
...
with st.sidebar:
    ...
    if page == "Dashboard":
        ...
        pass  # estimate picker moved to the main Fee Estimator page
    elif page == "Clients":
        st.header("Clients")
        ...
        if not allocs.empty:
            ...
            for m in member_names:
                ...
                if member_week_planned(m, cur_monday_str, allocs) > wcap: over.append(m)
            if over: st.warning("⚠️ Over capacity this week: " + ", ".join(over))
            else: st.caption("No one over capacity this week.")
        st.markdown("---")
    ...
    with col_add:
        ...
        if st.session_state.show_add_project:
            ...
            for it in n_ms_items:
                ...
                _d = mc3.date_input("date", value=_dv, key=f"new_ms_date_{it['id']}", label_visibility="collapsed", format="YYYY-MM-DD")
            ...
            if st.button("＋ Add milestone", key="new_ms_add"):
                n_ms_items.append({"id": create_id(), "text": "", "done": False, "date": None}); st.rerun()
...
# ── TASK TRACKER ──────────────────────────────────────────────────────────────
    ...
    if task_df.empty: st.info("No tasks match the filters.")
        ...
        for _proj_name in task_df["Project name"].fillna("Unassigned project").replace("", "Unassigned project").unique():
            with st.expander(f"📁 {_proj_name}", expanded=True):
                _proj_tasks = task_df[task_df["Project name"].fillna("Unassigned project").replace("", "Unassigned project") == _proj_name]
                cols = st.columns(3)
                for i, (_, task) in enumerate(_proj_tasks.iterrows()):
                    ...
                    with cols[i % 3]:
                        ...
                        if is_exp:
                            render_capacity_popover(member_names, f"edit_{tid}")
    ...
    render_capacity_popover(member_names, "newtask")
...
# ── TIMESHEETS ────────────────────────────────────────────────────────────────
    ...
    with tab_weekly:
        if not member_names:
            st.warning("No team members set up yet. Add team members in Resourcing → Team first.")
        else:
            ...
            for _, r in st.session_state.projects.iterrows():
                ...
                if wt_member in pmembers or wt_member == (r.get("Project manager", "") or ""):
                    if r["Project name"] not in seen_pn:
                        allocated.append((r["Project ID"], r["Project name"], r.get("Stage", ""))); seen_pn.add(r["Project name"])
            if not st.session_state.resource_allocs.empty:
                ma = st.session_state.resource_allocs[st.session_state.resource_allocs["Team member"] == wt_member]
                for pname in ma["Project name"].dropna().unique():
                    if pname in seen_pn: continue
                    prow = st.session_state.projects[st.session_state.projects["Project name"] == pname]
                    if not prow.empty:
            ...
            def build_initial_rows():
                rws = [{"rid": f"fixed_{n}", "pid": "", "pname": n, "phase": n, "fixed": True} for n in FIXED_NAMES]
                entry_combos = []
                for _, e in week_entries.iterrows():
                    if e["Project ID"] == "" and e["Project name"] in FIXED_NAMES: continue
                    combo = (e["Project ID"], e["Project name"], e["Phase"])
                    if combo not in entry_combos: entry_combos.append(combo)
                alloc_names = {a[1] for a in allocated}
                for pid, pname, stage in allocated:
                    matching = [c for c in entry_combos if c[1] == pname]
                    if matching:
                        for (epid, epn, eph) in matching:
                            rws.append({"rid": f"proj_{pid}_{eph}", "pid": pid, "pname": pname, "phase": eph, "fixed": False})
                    else:
                        rws.append({"rid": f"proj_{pid}", "pid": pid, "pname": pname, "phase": stage or "", "fixed": False})
                for (epid, epn, eph) in entry_combos:
                    if epn not in alloc_names:
                        rws.append({"rid": f"extra_{epn}_{eph}", "pid": epid, "pname": epn, "phase": eph, "fixed": False})
                return rws

            if wt_rows_key not in st.session_state:
                st.session_state[wt_rows_key] = build_initial_rows()
            ...
            if st.session_state.get(add_key, False) and not is_locked:
                with st.form(f"wt_add_form_{wt_member}_{week_start.strftime('%Y%m%d')}"):
                    ...
                    af = st.selectbox("Phase / Task", [""] + PHASE_OPTIONS, key="wt_add_phase")
                    c1, c2 = st.columns(2)
                    if c1.form_submit_button("Add row", use_container_width=True):
                        if ap and af:
                            ...
                            if not any(r["pname"] == ap and r["phase"] == af for r in rows):
                                st.session_state[wt_rows_key].append({"rid": f"add_{pid}_{af}_{create_id()}", "pid": pid, "pname": ap, "phase": af, "fixed": False})
                            st.session_state[add_key] = False; st.rerun()
                        else:
                            st.warning("Pick both a project and a phase.")
                    if c2.form_submit_button("Cancel", use_container_width=True):
                        st.session_state[add_key] = False; st.rerun()

            st.markdown("---")
            header_cols = st.columns([3, 2, 0.6] + [1] * 7 + [1])
            header_cols[0].markdown("**Job**")
            header_cols[1].markdown("**Phase**")
            for j, lbl in enumerate(day_labels):
                header_cols[j + 3].markdown(f"<div style='font-size:11px;font-weight:600;text-align:center'>{lbl}</div>", unsafe_allow_html=True)
            ...
            new_grid = {}; rows_to_remove = []
            for row in rows:
                rid = row["rid"]; pname = row["pname"]; is_fixed = row["fixed"]
                rcols = st.columns([3, 2, 0.6] + [1] * 7 + [1])
                rcols[0].markdown(f"<div style='font-size:13px;font-weight:600;color:#2c3e50;padding-top:6px'>{pname}</div>", unsafe_allow_html=True)
                if is_fixed:
                    rcols[1].markdown("<div style='font-size:11px;color:#888;padding-top:8px'>—</div>", unsafe_allow_html=True)
                    phase = row["phase"]
                else:
                    ph_idx = PHASE_OPTIONS.index(row["phase"]) if row["phase"] in PHASE_OPTIONS else 0
                    phase = rcols[1].selectbox("phase", PHASE_OPTIONS, index=ph_idx, key=f"wt_phase_{rid}", label_visibility="collapsed", disabled=is_locked)
                    row["phase"] = phase
                    if rcols[2].button("✕", key=f"wt_rm_{rid}", disabled=is_locked):
                        rows_to_remove.append(rid)
                new_grid[rid] = {}; row_total = 0.0
                for j, dk in enumerate(day_keys):
                    ...
                    init_val = parse_budget(match.iloc[0]["Hours"]) if not match.empty else 0.0
                    val = rcols[j + 3].number_input("h", min_value=0.0, max_value=24.0, step=0.25, value=init_val,
                        key=f"wt_cell_{rid}_{dk}", label_visibility="collapsed", format="%.2f", disabled=is_locked)
                    new_grid[rid][dk] = val; row_total += val; day_totals[dk] += val
                rcols[-1].markdown(f"<div style='text-align:center;font-weight:700;padding-top:6px'>{row_total:.2f}</div>", unsafe_allow_html=True)

            if rows_to_remove:
                st.session_state[wt_rows_key] = [r for r in rows if r["rid"] not in rows_to_remove]; st.rerun()

            st.markdown("---")
            tot_cols = st.columns([3, 2, 0.6] + [1] * 7 + [1])
            tot_cols[0].markdown("**Daily Total**")
            grand_total = 0.0
            for j, dk in enumerate(day_keys):
                tot_cols[j + 3].markdown(f"<div style='text-align:center;font-weight:700'>{day_totals[dk]:.2f}</div>", unsafe_allow_html=True)
            ...
            def collect_entries():
                ...
                for row in rows:
                    rid = row["rid"]; pname = row["pname"]; is_fixed = row["fixed"]; phase = row["phase"]
                    row_hours = sum(new_grid.get(rid, {}).get(dk, 0.0) for dk in day_keys)
                    if row_hours > 0 and not is_fixed and not phase:
                        missing = True; continue
                    for dk in day_keys:
                        hrs = new_grid.get(rid, {}).get(dk, 0.0)
                        if hrs > 0:
                            entries.append({"Entry ID": create_id(), "Project ID": row["pid"], "Project name": pname,
                                            "Phase": phase if not is_fixed else pname, "Team member": wt_member, "Role": mem_role,
                                            "Date": dk, "Hours": str(hrs), "Rate": "0.0" if is_fixed else str(mem_rate),
            ...
            def write_entries(entries):
                ...
                if entries: ts_df = pd.concat([ts_df, pd.DataFrame(entries)], ignore_index=True)
            ...
            if is_locked:
                ...
                if st.session_state.get("wt_show_unlock") and st.session_state.get("wt_unlock_member") == wt_member and st.session_state.get("wt_unlock_week") == week_start.strftime('%Y%m%d'):
                    ...
                    if st.button("🔓 Unlock to edit", key=f"show_unlock_{lock_key}"):
                ...
                if btn2.button("💾 Save Timesheet", use_container_width=True, key=f"wt_save_{wt_member}"):
                    ...
                    if missing: st.warning("Some project rows have hours but no phase selected — pick a phase before saving.")
                    else:
                        write_entries(entries); st.success(f"Timesheet saved — {grand_total:.2f} hrs logged for {wt_member}."); st.rerun()
                if btn3.button("📤 Submit Timesheet", use_container_width=True, key=f"wt_submit_{wt_member}"):
                    entries, missing = collect_entries()
                    if missing: st.warning("Some project rows have hours but no phase selected — pick a phase before submitting.")
                    else:
                        write_entries(entries); st.session_state[lock_key] = True; st.rerun()
...
# ── FEE ESTIMATOR ─────────────────────────────────────────────────────────────
    ...
    if not _ests.empty:
        for _, _e in _ests.iterrows():
            ...
            _label = f"{_e['Estimate name']}  -  ${_t['total']:,.0f}"
            if str(_e.get('Client','')).strip():
                _label += f"  ({_e['Client']})"
    ...
    if act == "new":
        ...
        if est_row.empty: st.warning("Estimate not found.")
            ...
            st.markdown("---")
            fee_breakdown.render_fee_breakdown(
                act, role_rates, DEFAULT_ROLES, save_estimate_lines, create_id)
            ...
            if st.button("📤 Push total fee to linked project & client", use_container_width=True):
                    ...
                    if has_proj:
                        ...
                        if not pidx2.empty:
                            st.session_state.projects.at[pidx2[0], "Fee"] = fee_str; save_projects(st.session_state.projects); pushed.append(f"project '{est.get('Project name','')}'")
                    if has_client:
                        cidx = st.session_state.companies[st.session_state.companies["Company ID"].astype(str) == str(est["Company ID"])].index
                        if not cidx.empty:
                            import re as _re
                            cur_notes = str(st.session_state.companies.at[cidx[0], "Notes"] or "")
                            tag = f"[Fee] Estimate '{est.get('Estimate name','')}':"
                            kept = [l for l in cur_notes.splitlines() if not l.strip().startswith(f"[Fee] Estimate '{est.get('Estimate name','')}':")]
                            kept.append(f"{tag} ${totals['total']:,.2f} (as of {pd.Timestamp.now().strftime('%Y-%m-%d')})")
                            st.session_state.companies.at[cidx[0], "Notes"] = "\n".join([l for l in kept if l.strip()])
                            save_companies(st.session_state.companies); pushed.append(f"client '{est.get('Client','')}'")
                    if pushed:
                        st.success(f"Fee of ${totals['total']:,.2f} pushed to " + " and ".join(pushed) + ".")
                    else:
                        st.warning("No linked project or client record found to push to.")
...
# ── RESOURCING ────────────────────────────────────────────────────────────────
    ...
    with tab_chart:
        ...
        st.markdown("")
        st.markdown(build_availability_table_html(member_names, weeks, st.session_state.resource_allocs, st.session_state.leave, hol_set, capacity), unsafe_allow_html=True)
    ...
    with tab_plan:
        if not member_names:
            st.warning("No team members set up yet. Add team members in the Team tab.")
        else:
            ...
            proj_filter = st.multiselect(f"Projects for {plan_member} — defaults to their allocations; add more if needed", all_project_names, default=default_show, key=f"res_proj_filter_{plan_member}")
            ...
            for j, w in enumerate(plan_weeks):
                ...
                _ld = member_week_leave_days(w["start"], _plan_leave, hol_set)
                marks = ""
            ...
            for ri, (pid, pname) in enumerate(all_rows):
                ...
                for j, w in enumerate(plan_weeks):
                    ws = w["start_str"]
                    new_grid[ri][ws] = rc[j + 1].number_input("h", min_value=0.0, max_value=80.0, step=0.5, value=get_planned(pname, ws),
                        key=f"res_cell_{ri}_{ws}_{plan_member}", label_visibility="collapsed", format="%.1f", disabled=is_director)
            ...
            for j, w in enumerate(plan_weeks):
                ...
                wcap, _, _ = member_week_capacity(w, capacity, _plan_leave, hol_set)
            ...
            if save_col.button("💾 Save Plan", use_container_width=True, key="res_save_plan", disabled=is_director):
                ...
                for ri, (pid, pname) in enumerate(all_rows):
                    for ws in week_strs:
                        hrs = new_grid[ri][ws]
                        if hrs > 0:
                            new_entries.append({"Alloc ID": create_id(), "Team member": plan_member, "Project ID": pid,
                                                "Project name": pname, "Week start": ws, "Projected hours": str(hrs),
    ...
    with tab_compare:
        ...
        if cmp_df[["Projected", "Actual"]].to_numpy().sum() == 0:
            ...
            bar = alt.Chart(long_df).mark_bar().encode(
                ...
                color=alt.Color("Type:N", scale=alt.Scale(domain=["Projected", "Actual"], range=["#3498db", "#2ecc71"]), legend=alt.Legend(title="", orient="top")),
            ...
            def style_variance(val):
                ...
                if v > 0: return "background-color:#fdecea;color:#c0392b"
                if v < 0: return "background-color:#eafaf1;color:#1e8449"
                return ""
            st.markdown("**Detail**")
        ...
        with st.form("role_rates_form"):
            ...
            for chunk_start in range(0, len(rr), 4):
                ...
                for j, role in enumerate(chunk):
                    rate_inputs[role] = rcols[j].number_input(role, min_value=0.0, step=5.0,
                        value=float(current_rates.get(role, DEFAULT_ROLES.get(role, 0.0))), key=f"role_rate_{role}")
                ...
                with st.form("add_leave_form"):
                    ...
                    if st.form_submit_button("Book leave", use_container_width=True):
                        if lv_to < lv_from:
                            ...
                            new_rows = []
                            d = pd.Timestamp(lv_from); end = pd.Timestamp(lv_to)
                            while d <= end:
                                if d.weekday() < 5 and d.date() not in existing:
                                    new_rows.append({"Leave ID": create_id(), "Team member": lv_member, "Date": d.strftime("%Y-%m-%d"), "Type": lv_type, "Notes": lv_notes})
                                d += pd.Timedelta(days=1)
                            if new_rows:
                                st.session_state.leave = pd.concat([st.session_state.leave, pd.DataFrame(new_rows)], ignore_index=True)
                                save_leave(st.session_state.leave); st.success(f"Booked {len(new_rows)} day(s) of leave for {lv_member}."); st.rerun()
                            else:
                ...
                if lv_df.empty:
                    ...
                    for (mem, typ), grp in lv_df.groupby(["Team member", "Type"]):
                        ...
                        for prev, cur_d in zip(dates, dates[1:]):
                            if (cur_d - prev).days <= 3: run.append(cur_d)
                            else: runs.append(run); run = [cur_d]
                        runs.append(run)
                        for r in runs:
                            shown += 1
                            label = r[0].strftime("%d %b") if len(r) == 1 else f"{r[0].strftime('%d %b')} – {r[-1].strftime('%d %b %Y')}"
                            rc = st.columns([3, 2, 1])
                            rc[0].markdown(f"**{mem}**"); rc[1].caption(f"{label} · {typ} ({len(r)}d)")
                            if rc[2].button("🗑", key=f"del_leave_{mem}_{typ}_{r[0].strftime('%Y%m%d')}"):
                                drop_strs = [d.strftime("%Y-%m-%d") for d in r]
                                st.session_state.leave = st.session_state.leave[~((st.session_state.leave["Team member"] == mem) & (st.session_state.leave["Type"] == typ) & (st.session_state.leave["Date"].isin(drop_strs)))]
                                save_leave(st.session_state.leave); st.rerun()
                    if shown == 0: st.caption("No upcoming leave.")
    ...
    with tab_holidays:
        ...
        if yc2.button("＋ Add year", use_container_width=True, key="add_year_hols"):
            ...
            if new_rows:
                ...
                save_holidays(st.session_state.holidays); st.success(f"Added {len(new_rows)} public holiday(s) for {int(year_choice)}."); st.rerun()
            else:
                st.info(f"All {int(year_choice)} public holidays are already in the list.")
        st.caption("Generates standard NZ public holidays (incl. computed Easter, King's Birthday, Labour Day and official Matariki dates). Edit or delete any below — e.g. add regional anniversary days.")
        st.markdown("---")
            ...
            with st.form("add_holiday_form"):
                ...
                if st.form_submit_button("Add holiday", use_container_width=True):
                    ...
                    if not st.session_state.holidays[st.session_state.holidays["Date"] == ds].empty:
                        ...
                        st.session_state.holidays = pd.concat([st.session_state.holidays, pd.DataFrame([{"Date": ds, "Name": h_name or "Holiday"}])], ignore_index=True)
